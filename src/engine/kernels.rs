...
pub fn spiky_kernel_gradient(pos: Vec2, pos_other: Vec2, params: &SimulationParams) -> Vec2 {
    ...
    let norm_coeff = -10.0 / (PI * params.influence_radius.powf(5.0)); // -45.0 / (PI * INFLUENCE_RADIUS.powf(6.0)) for 3D
    ...
    if r < 0.00001 {
        ...
        return norm_coeff * (params.influence_radius).powf(2.0) * random_dir;
    }
    let r_hat = delta / r;
    if r <= params.influence_radius {
        norm_coeff * (params.influence_radius - r).powf(2.0) * r_hat
    } else {
        vec2(0.0, 0.0)
    }
}
...
pub fn poly_kernel(pos: Vec2, pos_other: Vec2, params: &SimulationParams) -> f32 {
    ...
    let delta = pos - pos_other;
    let r = delta.length(); // magnitude of the vector pointing at particle i
    let norm_coeff = 4.0 / (PI * params.influence_radius.powf(8.0)); // 315.0 / (64.0 * PI * INFLUENCE_RADIUS.powf(9.0)) for 3D
    if r <= params.influence_radius {
        norm_coeff * (params.influence_radius.powf(2.0) - r.powf(2.0)).powf(3.0)
    } else {
        0.0
    }
}
